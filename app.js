...
function buildCatList(){
  ...
  cats.forEach((c,i)=>{
    ...
    wrap.innerHTML='<div class="ctop"><span class="cswatch" id="csw'+i+'" style="background:'+co+'"></span><input class="cname" id="cn'+i+'" type="text" value="'+c.name+'"><div class="cpwrap"><input class="cpct" id="cp'+i+'" type="number" min="0" max="200" value="'+c.pct+'"><span class="cpsym">%</span></div><button class="cdel" data-i="'+i+'"><i class="ti ti-x"></i></button></div><div class="cbarbg"><div class="cbarfill" id="cbf'+i+'" style="width:0%;background:'+co+'"></div></div><div class="camt" id="ca'+i+'"></div>';
    el.appendChild(wrap);
...
function rebuildACatList(){
  ...
  cats.forEach((c,i)=>{
    ...
    r.innerHTML='<input type="color" class="accol" value="'+co+'" style="background:'+co+'"><input class="acname" type="text" value="'+c.name+'"><input class="acpct" type="number" min="0" max="200" value="'+c.pct+'"><span style="font-size:12px;color:var(--txm)">%</span><button class="acdel" data-i="'+i+'"><i class="ti ti-trash"></i></button>';
    el.appendChild(r);
...
function startDataSync(uid){
  ...
  unsub=ref.onSnapshot(snap=>{
    if(snap.exists()){
      const d=snap.data();
      if(d._savedAt!==window._lastSaveTs) loadFromCloud(d);
    }
  });
}
...
function saveToCloud(){
  ...
  ref.set({cats,colors,baseMo,cur,incSplits:window._incSplits||[],paycheckHistory:window._paycheckHistory||[],_savedAt:ts},{merge:true})
...
// ── INIT ──
auth.onAuthStateChanged(user=>{ if(user) onUserSignedIn(user); else onUserSignedOut(); });
