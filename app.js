...
function buildCatList(){
  ...
  cats.forEach((c,i)=>{
    ...
    el.appendChild(wrap);
    document.getElementById('cn'+i).addEventListener('input', function(){ cats[i].name=this.value; updateDash(); scheduleSave(); });
    document.getElementById('cp'+i).addEventListener('input', function(){ cats[i].pct=parseFloat(this.value)||0; updateDash(); scheduleSave(); });
...
function aSetInc(v){ baseMo=parseFloat(v)||0; document.getElementById('incInput').value=Math.round(dispInc()).toLocaleString(); updateDash(); scheduleSave(); }
function aSetCur(v){ cur=v; document.getElementById('curSym').textContent=v; updateDash(); scheduleSave(); }
...
function applyPreset(p){
  ...
  cats=P[p].map(c=>({...c})); if(myChart){myChart.destroy();myChart=null;} buildCatList(); updateDash(); rebuildACatList(); rebuildCatColList(); scheduleSave(); toast('Preset applied!');
}
...
function resetAll(){
  ...
  if(myChart){myChart.destroy();myChart=null;} buildCatList(); updateDash(); rebuildACatList(); rebuildCatColList(); scheduleSave(); toast('Reset to defaults');
}
function rebuildACatList(){
  ...
  cats.forEach((c,i)=>{
    ...
    el.appendChild(r);
    r.querySelector('.accol').addEventListener('input',function(){ colors[i]=this.value; this.style.background=this.value; const sw=document.getElementById('csw'+i);if(sw)sw.style.background=this.value; const bar=document.getElementById('cbf'+i);if(bar)bar.style.background=this.value; rebuildCatColList(); updateDash(); scheduleSave(); });
    r.querySelector('.acname').addEventListener('input',function(){ cats[i].name=this.value; const n=document.getElementById('cn'+i);if(n)n.value=this.value; updateDash(); scheduleSave(); });
    r.querySelector('.acpct').addEventListener('input',function(){ cats[i].pct=parseFloat(this.value)||0; const p=document.getElementById('cp'+i);if(p)p.value=this.value; updateDash(); scheduleSave(); });
